...
static void rna_LayerCollectionEngineSettings_update(bContext *UNUSED(C), PointerRNA *ptr)
{
	 ...
	 * We need to do this because the LayerCollection may be coming from different ID types (Scene or Collection)
...
static void rna_LayerCollections_active_collection_set(PointerRNA *ptr, PointerRNA value)
{
	...
	if (index != -1) view_layer->active_collection = lc;
}
...
static void rna_def_layer_collection(BlenderRNA *brna)
{
	...
	RNA_def_property_boolean_negative_sdna(prop, NULL, "flag", LAYER_COLLECTION_EXCLUDE);
	RNA_def_property_clear_flag(prop, PROP_ANIMATABLE);
	RNA_def_property_ui_text(prop, "Use", "Use collection in this view layer");
