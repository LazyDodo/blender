    ...
    *mesh_islands = MEM_reallocN(*mesh_islands, islands * sizeof(MeshIsland *));
    ...
    else if (rmd->constraint_target == MOD_FRACTURE_VERTEX)
    {
        ...
        if (rmd->dm_group && rmd->use_constraint_group)
        {
            ...
            for (go = rmd->dm_group->gobject.first; go; go = go->next)
            {
                if ( go->ob != ob)
                {
                    ...
                    if (fmdi && fmdi->visible_mesh_cached) {
                        int v = fmdi->visible_mesh_cached->totvert;
                        int x = 0;
                        ...
                        memcpy(mvert + totvert, fmdi->visible_mesh_cached->mvert, v * sizeof(MVert));
            ...
            totvert = rmd->visible_mesh_cached->totvert;
            mvert = rmd->visible_mesh_cached->mvert;
        }
                              ...
                              KDTree **combined_tree, float co[3], Object *ob, Scene *scene)
{
    ...
    {
        Shard* s = BKE_fracture_shard_find(&rmd->frac_mesh->shard_map, mi->id);
        if (s->parent_id > -1) {
            return;
        }
    }
    ...
    for (i = 0; i < r; i++) {
        ...
        if ((mi != mi2) && (mi2 != NULL)) {
            ...
            if ((rmd->fracture_mode == MOD_FRACTURE_DYNAMIC))
            {
                Shard* s1 = BKE_fracture_shard_find(&rmd->frac_mesh->shard_map, mi->id);
                Shard* s2 = BKE_fracture_shard_find(&rmd->frac_mesh->shard_map, mi2->id);
